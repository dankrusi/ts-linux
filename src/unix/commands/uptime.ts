...
export const installUptime: UnixCommandInstaller = (ctx): void => {
  ...
  core({
        ...
        run: ({ args, sys }) => {
          ...
          const now = sys.now();
          const uptimeSeconds = shellUptimeSeconds();
          const pretty = formatDurationCompact(uptimeSeconds);
          const usersOnline = 1;
          const base = uptimeSeconds / 300;
          const load1 = clamp(0.12 + Math.sin(base) * 0.1 + Math.random() * 0.05, 0, 4);
          const load5 = clamp(0.1 + Math.cos(base / 2) * 0.08 + Math.random() * 0.04, 0, 4);
          const load15 = clamp(0.08 + Math.sin(base / 3) * 0.06 + Math.random() * 0.03, 0, 4);
          sys.write(
